...
# # ----------Все новости по языку--------
# class GetNewsList(View):
#     def get(self,request,language):
#         news = News.objects.filter(language=language).values()
#         news_data = list(news)
#         return JsonResponse({f'news_list_{language}': news_data})
#
#
# #--------------Новости по slug и по языку---------------
#
# class GetNewsDetail(View):
#     def get(self,request,slug,language):
#         news = News.objects.filter(language=language,slug=slug).values()
#         news_data = list(news)
#         return JsonResponse({f'news_detail_{language}': news_data})
#
#
# #-------------------Сортировка по категориям и нужному языку----------------------------
#
# class SortNewsByCategory(View):
#     def get(self, request, category, language):
#         category_mappings = {
#             'all_news': 'all_news',
#             'important': 'important',
...
#             news = News.objects.filter(language=language).values()
#         else:
#             news = News.objects.filter(category=category_key, language=language).values()
#
#         news_data = list(news)
#         return JsonResponse({f'category_{category}_news': news_data})
